...
除以上每次使用的统计情况之外，选取十月中的一周统计总车辆数和日单车使用次数、日平均单车使用时间如下表\ref{bikeusage}所示。
\vspace{3mm}    %空一行
\begin{table}[H]
	\centering	
	\caption{单车使用情况}
	...
	\begin
	{tabular}{p{15mm}<{\centering} p{15mm}<{\centering} p{15mm}<{\centering} p{15mm}<{\centering}}
		\toprule[2pt]
		Date & Used bikes & Average use time/min & Average use distance/km  \\
		\midrule[1pt]
		2017-10-09 & 14260 & 9.7 & 0.4\\
		2017-10-10 & 13867 & 10.0 & 0.5 \\
		2017-10-11 & 14836 & 10.1 & 0.4  \\
		2017-10-12 & 12750 & 9.7 & 0.4 \\
		2017-10-13 & 14790 & 10.3 & 0.4\\
		2017-10-14 & 17444 & 11.4 & 0.4 \\
		2017-10-15 & 17192 & 11.6 & 0.4 \\
		2017-10-16 & 15301 & 9.9 & 0.4 \\
		2017-10-17 & 15426 & 10.3 & 0.4 \\
		\bottomrule[2pt]
	\end{tabular}
\end{table}
\vspace{3mm}
...
选取几天的需求随时间变化规律展示如\ref{demand}所示。图中显示，工作日和周末的使用规律并不完全相同。在工作日，造成七点至十点、晚上五点到八点存在明显的使用高峰，而周末这种高峰规律会相对较弱，但在晚间仍然存在使用高峰。由于工作日和周末的规律不同，所以进行需求预测时，需要将两者分开建模，以提高模型预测的准确度。
\begin{figure}[H]
	\centering
...
在第一部的小型网罗实例中，我们挑选了工作日早高峰七点至十点的需求数据用作模型输入。假设在这个时间段内的任何时间切片中都可以进行调度。因为时间以五分钟进行切片，每个站点的单车需求也以五分钟为时间段进行计数。虽然由于可能有未被满足的需求的存在，即消费者到达期望出发点时没有找到可用单车，所以从单车使用数据中得到的单车需求量并不严格等于真实需求，但是由于从开始的单车分布结果中可以看出，义顺区内的大部分区域单车存在过饱和，所以需求大于供给的情况可以忽略不计，所以在进行输入预测时仍将从数据中统计得到的使用量视为需求量。在选取十个站点时，选取三个小时内使用量最高的十个站点作为研究范围，以减少随机需求的影响。
